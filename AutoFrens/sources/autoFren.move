module MasterFrenDepolyer::AutoAni {
    use std::signer;
    use aptos_std::event;
    use aptos_framework::coin::{Self};
    use aptos_framework::timestamp;
    use aptos_framework::account::{Self, SignerCapability};
    use MasterFrenDepolyer::KekMasterFrenV1::{Self, KEK};
    ...
    const KEK_PRECISION: u128 = 1000000000000;  // 1e12
    ...
    struct UserInfo has key, store {
        ...
        last_user_action_KEK: u64,   // keeps track of KEK deposited at the last user action
        last_user_action_time: u64,  // keeps track of the last user action time
    }

    struct AutoKEKData has drop, key {
    ...
    fun get_resource_account(): signer acquires AutoKEKData {
        let signer_cap = &borrow_global<AutoKEKData>(DEPLOYER).signer_cap;
        account::create_signer_with_capability(signer_cap)
    }

    // return pair admin account address
    fun get_resource_account_address(): address acquires AutoKEKData {
    ...
    fun init_module(admin: &signer) {
        ...
        // register
        KekMasterFrenV1::register_KEK(admin);
    ...
    fun when_paused() acquires AutoKEKData {
        assert!(borrow_global<AutoKEKData>(DEPLOYER).is_pause == true, PAUSABLE_ERROR);
    }

    fun when_not_paused() acquires AutoKEKData {
        assert!(borrow_global<AutoKEKData>(DEPLOYER).is_pause == false, PAUSABLE_ERROR);
    }

    // Deposits funds into the KEK Vault
    public entry fun deposit(
        account: &signer,
        amount: u64
    ) acquires AutoKEKData, UserInfo, Events {
        when_not_paused();
        assert!(amount > 0, AMOUNT_ERROR);
        let auto_data = borrow_global_mut<AutoKEKData>(DEPLOYER);
        ...
        coin::transfer<KEK>(account, resource_account_address, amount);
        let current_shares;
        ...
        if (exists<UserInfo>(acc_addr)) {
            ...
            user_info.last_user_action_KEK = (user_info.shares * (balance_of(resource_account_address) as u128) / auto_data.total_shares as u64);
            user_info.last_user_action_time = timestamp::now_seconds();
        } else {
            let user_info = UserInfo {
                shares: current_shares,
                last_deposited_time: timestamp::now_seconds(),
                last_user_action_KEK: (current_shares * (balance_of(resource_account_address) as u128) / auto_data.total_shares as u64),
    ...
    public entry fun withdraw(
        ...
        if (bal < current_amount) {
            ...
            KekMasterFrenV1::leave_staking(&resource_account_signer, (bal_withdraw as u64));
        ...
        if (timestamp::now_seconds() < user_info.last_deposited_time + auto_data.withdraw_fee_period) {
            ...
            coin::transfer<KEK>(&resource_account_signer, auto_data.treasury, (current_withdraw_fee as u64));
        ...
        if (user_info.shares > 0) {
            ...
            user_info.last_user_action_KEK = 0;
        };

        user_info.last_user_action_time = timestamp::now_seconds();
        coin::transfer<KEK>(&resource_account_signer, acc_addr, (current_amount as u64));
        // emit withdraw
        let events = borrow_global_mut<Events>(resource_account_address);
    ...
    public entry fun harvest(
        ...
        KekMasterFrenV1::leave_staking(&resource_account_signer, 0);
        let bal = (available(resource_account_address) as u128);
        let current_performance_fee = bal * (auto_data.performance_fee as u128) / 10000;
        coin::transfer<KEK>(&resource_account_signer, auto_data.treasury, (current_performance_fee as u64));

        let current_call_fee = bal * (auto_data.call_fee as u128) / 10000;
        coin::transfer<KEK>(&resource_account_signer, acc_addr, (current_call_fee as u64));
    ...
    fun earn_internal(
        ...
        if (bal > 0) {
            KekMasterFrenV1::enter_staking(resource_account_signer, bal);
        }
    }
    ...
    fun balance_of(
        ...
        let amount = KekMasterFrenV1::get_user_info_amount<KEK>(addr);
        coin::balance<KEK>(addr) + amount
    }

    public entry fun set_admin_address(
        admin: &signer,
        new_admin_address: address
    ) acquires AutoKEKData {
        let aa_data = borrow_global_mut<AutoKEKData>(DEPLOYER);
    ...
    public entry fun emergency_withdraw(
        ...
        let aa_data = borrow_global_mut<AutoKEKData>(DEPLOYER);
        assert!(signer::address_of(admin) == aa_data.admin_address, FORBIDDEN);
        KekMasterFrenV1::emergency_withdraw<KEK>(&get_resource_account());
    }
    ...
    public fun calculate_harvest_KEK_rewards(): u64 acquires AutoKEKData {
        let resource_account_signer = get_resource_account();
        let resource_account_address = signer::address_of(&resource_account_signer);
        let auto_data = borrow_global<AutoKEKData>(DEPLOYER);

        let amount = KekMasterFrenV1::pending_KEK<KEK>(resource_account_address);
    ...
    public fun calculate_total_pending_KEK_rewards(): u64 acquires AutoKEKData {
        let resource_account_signer = get_resource_account();
        let resource_account_address = signer::address_of(&resource_account_signer);
    ...
    public fun get_price_per_full_share(): u64 acquires AutoKEKData {
        let resource_account_signer = get_resource_account();
        let resource_account_address = signer::address_of(&resource_account_signer);
        let auto_data = borrow_global<AutoKEKData>(DEPLOYER);
        
        if (auto_data.total_shares == 0) {
            return (KEK_PRECISION as u64)
        } else {
            let amount = (balance_of(resource_account_address) as u128) * KEK_PRECISION / auto_data.total_shares;
            return (amount as u64)
        }
    }

    public fun get_auto_KEK_data(): (u128, u64, u64, u64, u64, u64) acquires AutoKEKData {
        let auto_data = borrow_global<AutoKEKData>(DEPLOYER);
    ...
    public fun test_init(creator: &signer, someone_else: &signer) {
        KekMasterFrenV1::test_init(creator, someone_else);
        init_module(creator);
    }

    #[test_only]
    public fun test_init_another(creator: &signer, someone_else: &signer, another_one: &signer) {
        KekMasterFrenV1::test_init_another(creator, someone_else, another_one);
        init_module(creator);
    }

    #[test(creator = @MasterFrenDepolyer, someone_else = @0x11)]
    public entry fun test_deposit_withdraw_1(creator: &signer, someone_else: &signer) acquires AutoKEKData, UserInfo, Events {
        test_init(creator, someone_else);
        let amount = 100000000;
        let (_total_shares, _performance_fee, _call_fee, withdraw_fee, _withdraw_fee_period, _last_harvested_time) = get_auto_KEK_data();

        deposit(someone_else, amount);
        assert!(coin::balance<KEK>(signer::address_of(someone_else)) == INIT_COIN - amount, TEST_ERROR);
        withdraw_all(someone_else);
        assert!(coin::balance<KEK>(signer::address_of(someone_else)) == INIT_COIN - amount * withdraw_fee / 10000, TEST_ERROR);
    }

    #[test(creator = @MasterFrenDepolyer, someone_else = @0x11)]
    public entry fun test_deposit_withdraw_2(creator: &signer, someone_else: &signer) acquires AutoKEKData, UserInfo, Events {
        test_init(creator, someone_else);
        let amount = 100000000;
        let (_total_shares, _performance_fee, _call_fee, _withdraw_fee, _withdraw_fee_period, _last_harvested_time) = get_auto_KEK_data();

        deposit(someone_else, amount);
        assert!(coin::balance<KEK>(signer::address_of(someone_else)) == INIT_COIN - amount, TEST_ERROR);
        timestamp::fast_forward_seconds(_withdraw_fee_period + 1);
        withdraw_all(someone_else);
        assert!(coin::balance<KEK>(signer::address_of(someone_else)) == INIT_COIN, TEST_ERROR);
    }

    #[test(creator = @MasterFrenDepolyer, someone_else = @0x11)]
    public entry fun test_deposit_withdraw_3(creator: &signer, someone_else: &signer) acquires AutoKEKData, UserInfo, Events {
        test_init(creator, someone_else);
        let amount = 100000000;

        deposit(someone_else, amount * 2);
        assert!(coin::balance<KEK>(signer::address_of(someone_else)) == INIT_COIN - 2 * amount, TEST_ERROR);
        timestamp::fast_forward_seconds(86400 * 10);
        withdraw(someone_else, (amount as u128));
        withdraw_all(someone_else);
        assert!(coin::balance<KEK>(signer::address_of(someone_else)) == INIT_COIN + 86400 * 10 * 9 / 10 * 100000000, TEST_ERROR);
    }

    #[test(creator = @MasterFrenDepolyer, someone_else = @0x11, another_one = @0x12)]
    public entry fun test_two_account_1(creator: &signer, someone_else: &signer, another_one: &signer) acquires AutoKEKData, UserInfo, Events {
        test_init_another(creator, someone_else, another_one);
        let amount = 100000000;

        deposit(someone_else, amount);
        KekMasterFrenV1::enter_staking(another_one, amount);
        assert!(coin::balance<KEK>(signer::address_of(someone_else)) == INIT_COIN - amount, TEST_ERROR);
        assert!(coin::balance<KEK>(signer::address_of(another_one)) == INIT_COIN - amount, TEST_ERROR);
        ...
        timestamp::fast_forward_seconds(86400 * 5);
        withdraw_all(someone_else);
        KekMasterFrenV1::leave_staking(another_one, amount);
        assert!(coin::balance<KEK>(signer::address_of(someone_else)) > coin::balance<KEK>(signer::address_of(another_one)), TEST_ERROR);
    }

    #[test(creator = @MasterFrenDepolyer, someone_else = @0x11, another_one = @0x12)]
    public entry fun test_two_account_2(creator: &signer, someone_else: &signer, another_one: &signer) acquires AutoKEKData, UserInfo, Events {
        test_init_another(creator, someone_else, another_one);
        let amount = 100000000;
    ...
    #[test(creator = @MasterFrenDepolyer, someone_else = @0x11, another_one = @0x12)]
    public entry fun test_two_account_3(creator: &signer, someone_else: &signer, another_one: &signer) acquires AutoKEKData, UserInfo, Events {
        test_init_another(creator, someone_else, another_one);
        let amount = 100000000;
        ...
        deposit(someone_else, amount);
        KekMasterFrenV1::enter_staking(another_one, amount);
        assert!(KekMasterFrenV1::pending_KEK<KEK>(signer::address_of(another_one)) == 0, TEST_ERROR);

        timestamp::fast_forward_seconds(86400 * 5);
        KekMasterFrenV1::leave_staking(another_one, amount * 2);
        withdraw_all(someone_else);
        assert!(coin::balance<KEK>(signer::address_of(someone_else)) < coin::balance<KEK>(signer::address_of(another_one)), TEST_ERROR);
    }
    ...
    public entry fun test_pause_1(creator: &signer, someone_else: &signer) acquires AutoKEKData, UserInfo, Events {
        test_init(creator, someone_else);
        pause(creator);
    ...
    #[test(creator = @MasterFrenDepolyer, someone_else = @0x11)]
    public entry fun test_pause_2(creator: &signer, someone_else: &signer) acquires AutoKEKData, UserInfo, Events {
        test_init(creator, someone_else);
        pause(creator);
        unpause(creator);
        deposit(someone_else, 10000);
